...
def setup_sighandlers():
    for i in [x for x in dir(signal) if x.startswith("SIG")]:
        try:
            if i == 'SIGKILL' or i == 'SIGSTOP':
                continue
            signum = getattr(signal, i)
            signal.signal(signum, signal.SIG_DFL)
        except:
            logging.error("Signal handler setup error!")
...
def db_insert_new_job(job_id, execution, cleanup, retries, hostname):
    db_execute("call spInsertNewJobRun('%s','%s','%s','%d','%s','%d');" % (job_id, execution, cleanup, retries, hostname, int(os.getpid())))
...
def run(gearman_worker, gearman_job):
    ...
    while status is None or status != 'COMPLETED':
        if attempts < retries:
            if status == 'STARTED':
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info("Registering task run..")
