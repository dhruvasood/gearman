...
def setup_workflow_in_db(name, num_steps):
    row = db_execute_select_one("call spInsertNewWorkflow('%s','%s', '%d', '%d');" % (name, socket.gethostname(), int(os.getpid()), num_steps))
    return row[0]
            ...
            try:
                ...
                while num <= len(steps):
                    try:
                        ...
                        print "Sending job %d with param: %s..." % (num,str(param))
                        job_request = gm_client.submit_job("run", param)
                        dir = GEARMAN_DIR + '/' + job_request.gearman_job.unique
                        logging.info("Step: %d response: %s" % (num,str(job_request.result)))
                        if job_request.result != "0":
                            print "Job %d failed!" % num
                            raise Exception("Error: step %d failed!" % num)
                        else:
                            print "Job %d is successful!" % num
                        stdio[num]={}
                ...
                db_set_workflow_failed(workflow_id)
                raise
            finally:
                gm_client.shutdown()
    except (gearman.errors, gearman.errors.ServerUnavailable):
